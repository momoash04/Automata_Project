...
def check_dfa(input_str):
    if not input_str:
        return {"accepted": False, "final_state": 0, "error": False}
    
    state = 0
    for c in input_str:
        if c == '0':
            transitions = {0:1, 1:1, 2:3, 3:3, 4:5, 5:5, 6:7, 7:7}
            state = transitions.get(state, 0)
        elif c == '1':
            transitions = {0:2, 1:2, 2:4, 3:4, 4:6, 5:6, 6:2, 7:2}
            state = transitions.get(state, 0)
        else:
            return {"accepted": False, "final_state": -1, "error": True}
    
    return {"accepted": state == 7, "final_state": state, "error": False}
...
class DFACanvas(QWidget):
    ...
    def draw_edge_line(self, painter, centers, _from, _to, label, curve_offset):
        ...
        if _from == _to:
            ...
            painter.drawText(int(c_from.x() - 5), int(c_from.y() - 65), label)
        ...
        painter.setPen(QColor(255, 165, 0) if is_highlighted else QColor(255, 255, 255))
        painter.drawText(int(cx - 5), int(cy - 10), label)
    ...
    def paintEvent(self, event):
        ...
        for i in range(8):
            ...
            text_color = QColor(0, 0, 0) if (i == self.current_state_index and not self.is_error and i != 7) else QColor(255, 255, 255)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.input_edit.setStyleSheet("background-color: #2d2d2d; color: #dcdcdc; border: 1px solid #555; padding: 4px;")
